...
def get_redirected_url(url, headers=None):
    try:
        # 禁用代理和 SSL 验证
        proxies = {
            "http": None,
        ...
        response = requests.get(url, headers=headers, allow_redirects=True, proxies=proxies, verify=False)
        print(f"Final URL: {url}")
        print(f"Redirected URL: {response.url}")
        return response.url
    except Exception as e:
        print(f"Failed to get redirected URL for {url}: {e}")
        return url  # 返回原始 URL 以继续处理
...
def save_as_markdown(note, data, save_dir,noteId,cookie):
    headers = {'Cookie': unquote(cookie)}
    note_id = note.get('id')
    title, content = data
    ...
    for file_id in matches:
        original_format = f"☺ {file_id}<0/></>"
        image_url = f"https://i.mi.com/file/full?type=note_img&fileid={file_id}"
        redirected_url = get_redirected_url(image_url, headers)
        content = content.replace(original_format, f"![image]({redirected_url})")
...
def main():
    ...
    for note in notes:
        ...
        save_as_markdown(note, content, save_dir,noteId,minote_cookie)
